(function (global, factory) {
  ...
  (global.RangeTrack = factory());
}(this, (function () { 'use strict';

  const VERSION = "0.1.1";
  const DEPENDENCIES = {"higlass":"1.6.0"};
  ...
  var RangeTrack = function RangeTrack(HGC) {
    if (!(this instanceof RangeTrack ? this.constructor : void 0)) {
    ...
    var RangeTrackClass =
    /*#__PURE__*/
    function (_HGC$tracks$BarTrack) {
      _inherits(RangeTrackClass, _HGC$tracks$BarTrack);

      function RangeTrackClass(context, options) {
        var _this;

        _classCallCheck(this, RangeTrackClass);

        _this = _possibleConstructorReturn(this, _getPrototypeOf(RangeTrackClass).call(this, context, options));
        _this.maxAndMin = {
          max: null,
          min: null
        };
        _this.mode = options.mode === 'whisker' ? 'whisker' : 'minMax';
        return _this;
      }

      _createClass(RangeTrackClass, [{
        key: "rerender",
        value: function rerender(options, force) {
          this.mode = options.mode === 'whisker' ? 'whisker' : 'minMax';

          _get(_getPrototypeOf(RangeTrackClass.prototype), "rerender", this).call(this, options, force);
        }
        /**
        ...
        value: function renderTile(tile) {
          ...
          var _this$getTilePosAndDi = this.getTilePosAndDimensions(tile.tileData.zoomLevel, tile.tileData.tilePos, this.tilesetInfo.bins_per_dimension || this.tilesetInfo.tile_size),
              tileX = _this$getTilePosAndDi.tileX,
              tileWidth = _this$getTilePosAndDi.tileWidth;

          var tileValues = tile.tileData.dense;
          if (tileValues.length === 0) return; // equal to the smallest non-zero value

          var _this$makeValueScale = this.makeValueScale(this.minVisibleValue(), this.medianVisibleValue, this.maxValue(), 0),
              _this$makeValueScale2 = _slicedToArray(_this$makeValueScale, 1),
              valueScale = _this$makeValueScale2[0];

          this.valueScale = valueScale;
          var colorScale = valueScale.copy();
          ...
          var tileXScale = linear$1().domain([0, this.tilesetInfo.tile_size || this.tilesetInfo.bins_per_dimension]).range([tileX, tileX + tileWidth]);
        ...
        key: "renderMinMax",
        value: function renderMinMax(tile, tileXScale) {
          var tileValues = tile.tileData.dense;
          var dataStepSize = tile.tileData.size;
          var resStepSize = +this.options.resolution || 1;
          var color = this.options.minMaxColor || 'grey';
          var colorHex = colorToHex(color);
          var opacity = +this.options.minMaxOpacity || 1;
          ...
          var yEndPos;
          var height;
          var numVals = tileValues.length / dataStepSize / resStepSize;
          ...
          for (var i = 0; i < numVals; i++) {
            ...
            this.addSVGInfo(tile, xPos, yEndPos, width, height, color); // this data is in the last tile and extends beyond the length
        ...
        key: "renderWhisker",
        value: function renderWhisker(tile, tileXScale) {
          var tileValues = tile.tileData.dense;
          var dataStepSize = tile.tileData.size;
          var resStepSize = +this.options.resolution || 1;
          var vLineG = new PIXI.Graphics();
          var minMaxG = new PIXI.Graphics();
          var stdG = new PIXI.Graphics();
          var meanG = new PIXI.Graphics();
          tile.graphics.addChild(vLineG);
          tile.graphics.addChild(minMaxG);
          tile.graphics.addChild(stdG);
          tile.graphics.addChild(meanG);
          var vLineColor = this.options.vLineColor || 'black';
          var vLineColorHex = colorToHex(vLineColor);
          var vLineOpacity = +this.options.vLineOpacity >= 0 ? +this.options.vLineOpacity : 1;
          vLineG.beginFill(vLineColorHex, vLineOpacity);
          var stdFillColor = this.options.stdFillColor || 'white';
          var stdFillColorHex = colorToHex(stdFillColor);
          var stdFillOpacity = +this.options.stdFillOpacity >= 0 ? +this.options.stdFillOpacity : 1;
          stdG.beginFill(stdFillColorHex, stdFillOpacity);
          var stdStrokeColor = this.options.stdStrokeColor || 'black';
          var stdStrokeColorHex = colorToHex(stdStrokeColor);
          var stdStrokeOpacity = +this.options.stdStrokeOpacity >= 0 ? +this.options.stdStrokeOpacity : 1;
          stdG.lineStyle(1, stdStrokeColorHex, stdStrokeOpacity);
          var minMaxColor = this.options.minMaxColor || 'black';
          var minMaxColorHex = colorToHex(minMaxColor);
          var minMaxOpacity = +this.options.minMaxOpacity >= 0 ? +this.options.minMaxOpacity : 1;
          minMaxG.beginFill(minMaxColorHex, minMaxOpacity);
          var meanColor = this.options.meanColor || 'black';
          var meanColorHex = colorToHex(meanColor);
          var meanOpacity = +this.options.meanOpacity >= 0 ? +this.options.meanOpacity : 1;
          ...
          for (var i = 0; i < numVals; i++) {
            ...
            if (resStepSize > 1) {
              ...
              for (var _j = 0; _j < resStepSize; _j++) {
                stdNewVal += Math.pow(meanVals[_j] - meanVal, 2);
              }
            ...
            std = Math.abs(this.dimensions[1] - this.valueScale(stdVal)); // this data is in the last tile and extends beyond the length
            // of the coordinate system
        ...
        value: function tileToLocalId(tile) {
          return "".concat(tile.join('.'), ".").concat(this.mode);
        }
      }, {
        key: "tileToRemoteId",
        ...
        value: function setDimensions(newDimensions) {
          ...
          _get(_getPrototypeOf(RangeTrackClass.prototype), "setDimensions", this).call(this, newDimensions);
    ...
    return _construct(RangeTrackClass, args);
  };

  var icon = '<svg width="100%" height="100%" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><rect x="11" y="0" width="11" height="1"/><rect x="16" y="1" width="1" height="30"/><rect x="11" y="7" width="11" height="21"/><rect x="12" y="8" width="9" height="19" fill="white"/><rect x="12" y="17" width="9" height="1"/><rect x="11" y="31" width="11" height="1"/></svg>'; // default
  ...
  RangeTrack.config = {
    ...
    availableOptions: ['mode', 'resolution', 'minMaxColor', 'minMaxOpacity', 'minColor', 'minOpacity', 'maxColor', 'maxOpacity', 'meanColor', 'meanOpacity', 'stdFillColor', 'stdFillOpacity', 'stdStrokeColor', 'stdStrokeOpacity', 'vLineColor', 'vLineOpacity'],
    defaultOptions: {
      mode: 'minMax',
      resolution: 1,
      minMaxColor: 'black',
