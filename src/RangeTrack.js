...
const BasicMultipleBarChart = (HGC, ...args) => {
  ...
  // const mix = HGC.libraries.mix;

  class RangeTrack extends mix(HGC.tracks.BarTrack).with(
    HGC.tracks.OneDimensionalMixin
  ) {
    ...
    renderTile(tile) {
      ...
      const localGraphics = new PIXI.Graphics();
      ...
      const colorScale =
        this.options.colorScale || scaleOrdinal(schemeCategory10);
      const width =
        this._xScale(tileX + tileWidth / this.tilesetInfo.tile_size) -
        this._xScale(tileX);
